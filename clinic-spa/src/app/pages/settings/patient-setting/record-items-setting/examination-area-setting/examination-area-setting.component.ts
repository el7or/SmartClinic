...
@Component({
  selector: 'examination-area-setting',
  templateUrl: './examination-area-setting.component.html',
  styleUrls: ['./examination-area-setting.component.scss']
})
export class ExaminationAreaSettingComponent implements OnInit ,OnDestroy {
  formLoading = false;
  ...
  ngOnInit() {
    ...
    this.subs.add(
      this.recordItemService.getItemValues(ItemsType.ExaminationArea).subscribe(
            ...
            if (this.itemValues.some((i) => i.text.trim() == itemText.trim())) {
              ...
              this.subs.add(
                this.recordItemService
                  .postItemValues(itemObj, ItemsType.ExaminationArea)
                  .subscribe(
                    (res: AnyPatientFileValue) => {
                      if (this.itemValues) {
        ...
        .onClose.subscribe((itemText: string) => {
          if (itemText && itemText.trim() != item.text.trim()) {
            if (
              ...
              this.subs.add(
                this.recordItemService
                  .putItemValues(item, ItemsType.ExaminationArea)
                  .subscribe(
                    () => {
                      this.doneSwal.fire();
                      this.formLoading = false;
  ...
  onDelete(itemId: number, index) {
    this.deleteSwal.fire().then((result) => {
      if (result.value) {
        ...
        this.subs.add(
          this.recordItemService
            .deleteItemValues(itemId, ItemsType.ExaminationArea)
            .subscribe(
              () => {
                this.formLoading = false;
                this.doneSwal.fire();
