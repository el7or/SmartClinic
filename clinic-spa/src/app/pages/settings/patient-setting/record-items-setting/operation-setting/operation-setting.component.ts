...
@Component({
  selector: 'operation-setting',
  templateUrl: './operation-setting.component.html',
  styleUrls: ['./operation-setting.component.scss']
})
export class OperationSettingComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.subs.add(
      this.recordItemService.getItemValues(ItemsType.Operation).subscribe(
            ...
            if (this.itemValues.some((i) => i.text.trim() == itemText.trim())) {
              ...
              this.subs.add(
                this.recordItemService
                  .postItemValues(itemObj, ItemsType.Operation)
                  .subscribe(
                    (res: AnyPatientFileValue) => {
                      if (this.itemValues) {
        ...
        .onClose.subscribe((itemText: string) => {
          if (itemText && itemText.trim() != item.text.trim()) {
            if (
              ...
              this.subs.add(
                this.recordItemService
                  .putItemValues(item, ItemsType.Operation)
                  .subscribe(
                    () => {
                      this.doneSwal.fire();
                      this.formLoading = false;
  ...
  onDelete(itemId: number, index) {
    this.deleteSwal.fire().then((result) => {
      if (result.value) {
        ...
        this.subs.add(
          this.recordItemService
            .deleteItemValues(itemId, ItemsType.Operation)
            .subscribe(
              () => {
                this.formLoading = false;
                this.doneSwal.fire();
