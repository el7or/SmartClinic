...
@Component({
  selector: 'ray-area-setting',
  templateUrl: './ray-area-setting.component.html',
  styleUrls: ['./ray-area-setting.component.scss']
})
export class RayAreaSettingComponent implements OnInit,OnDestroy {
  formLoading = false;
  ...
  ngOnInit() {
    ...
    this.subs.add(
      this.recordItemService.getItemValues(ItemsType.RayArea).subscribe(
            ...
            if (this.itemValues.some((i) => i.text.trim() == itemText.trim())) {
              ...
              this.subs.add(
                this.recordItemService
                  .postItemValues(itemObj, ItemsType.RayArea)
                  .subscribe(
                    (res: AnyPatientFileValue) => {
                      if (this.itemValues) {
        ...
        .onClose.subscribe((itemText: string) => {
          if (itemText && itemText.trim() != item.text.trim()) {
            if (
              ...
              this.subs.add(
                this.recordItemService
                  .putItemValues(item, ItemsType.RayArea)
                  .subscribe(
                    () => {
                      this.doneSwal.fire();
                      this.formLoading = false;
  ...
  onDelete(itemId: number, index) {
    this.deleteSwal.fire().then((result) => {
      if (result.value) {
        ...
        this.subs.add(
          this.recordItemService
            .deleteItemValues(itemId, ItemsType.RayArea)
            .subscribe(
              () => {
                this.formLoading = false;
                this.doneSwal.fire();
