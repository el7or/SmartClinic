...
@Component({
  selector: 'therapy-area-setting',
  templateUrl: './therapy-area-setting.component.html',
  styleUrls: ['./therapy-area-setting.component.scss']
})
export class TherapyAreaSettingComponent implements OnInit,OnDestroy {
  formLoading = false;
  ...
  ngOnInit() {
    ...
    this.subs.add(
      this.recordItemService.getItemValues(ItemsType.PhysicalTherapyArea).subscribe(
            ...
            if (this.itemValues.some((i) => i.text.trim() == itemText.trim())) {
              ...
              this.subs.add(
                this.recordItemService
                  .postItemValues(itemObj, ItemsType.PhysicalTherapyArea)
                  .subscribe(
                    (res: AnyPatientFileValue) => {
                      if (this.itemValues) {
        ...
        .onClose.subscribe((itemText: string) => {
          if (itemText && itemText.trim() != item.text.trim()) {
            if (
              ...
              this.subs.add(
                this.recordItemService
                  .putItemValues(item, ItemsType.PhysicalTherapyArea)
                  .subscribe(
                    () => {
                      this.doneSwal.fire();
                      this.formLoading = false;
  ...
  onDelete(itemId: number, index) {
    this.deleteSwal.fire().then((result) => {
      if (result.value) {
        ...
        this.subs.add(
          this.recordItemService
            .deleteItemValues(itemId, ItemsType.PhysicalTherapyArea)
            .subscribe(
              () => {
                this.formLoading = false;
                this.doneSwal.fire();
