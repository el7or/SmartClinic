...
@Component({
  selector: 'therapy-setting',
  templateUrl: './therapy-setting.component.html',
  styleUrls: ['./therapy-setting.component.scss']
})
export class TherapySettingComponent implements OnInit ,OnDestroy {
  formLoading = false;
  ...
  ngOnInit() {
    ...
    this.subs.add(
      this.recordItemService.getItemValues(ItemsType.PhysicalTherapy).subscribe(
            ...
            if (this.itemValues.some((i) => i.text.trim() == itemText.trim())) {
              ...
              this.subs.add(
                this.recordItemService
                  .postItemValues(itemObj, ItemsType.PhysicalTherapy)
                  .subscribe(
                    (res: AnyPatientFileValue) => {
                      if (this.itemValues) {
        ...
        .onClose.subscribe((itemText: string) => {
          if (itemText && itemText.trim() != item.text.trim()) {
            if (
              ...
              this.subs.add(
                this.recordItemService
                  .putItemValues(item, ItemsType.PhysicalTherapy)
                  .subscribe(
                    () => {
                      this.doneSwal.fire();
                      this.formLoading = false;
  ...
  onDelete(itemId: number, index) {
    this.deleteSwal.fire().then((result) => {
      if (result.value) {
        ...
        this.subs.add(
          this.recordItemService
            .deleteItemValues(itemId, ItemsType.PhysicalTherapy)
            .subscribe(
              () => {
                this.formLoading = false;
                this.doneSwal.fire();
